...
- **Permutation methods:** ter Braak (1992) residual permutation,
  Kennedy (1995) individual exposure-residual permutation, and Kennedy
  (1995) joint F-style test.
- **Vectorised OLS** via batch pseudoinverse multiplication — the
  design matrix is inverted once and applied to all permuted response
  vectors simultaneously.
- **Optional JAX backend** for logistic regression: `jax.vmap` over a
...
- **Unique permutation pre-generation** with hash-based deduplication
  for large *n* and exhaustive enumeration for small *n* (≤ 12).
- **Phipson & Smyth (2010) corrected p-values** that are guaranteed
  never to be exactly zero.
- **Confounder identification pipeline:** Pearson correlation screening
...
- **Formatted ASCII table output** modeled after statsmodels, with
  separate layouts for individual-coefficient and joint tests.
- **Development infrastructure:** Google-style docstrings, PEP 561
...
- Accept `numpy` arrays, `pandas` DataFrames/Series, and (optionally)
